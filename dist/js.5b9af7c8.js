...
var Game = function (_Phaser$Scene) {
  ...
  function Game() {
    ...
    _this.currentPlayers = _this.currentPlayers.bind(_this);
    _this.addPlayer = _this.addPlayer.bind(_this);
    _this.addOtherPlayers = _this.addOtherPlayers.bind(_this);
    _this.playerMoved = _this.playerMoved.bind(_this);
    _this.disconnected = _this.disconnected.bind(_this);
    _this.scoreUpdate = _this.scoreUpdate.bind(_this);
    _this.starLocation = _this.starLocation.bind(_this);
    return _this;
  }
  ...
  _createClass(Game, [{
    ...
    value: function preload() {
      this.load.image('person', 'img/character.jpg');
      this.load.image('otherPlayer', 'img/character.jpg');
      this.load.image('star', 'img/star-icon.png');
      this.load.image('tiles', 'sprites/desert.png');
    ...
    value: function create() {
      ...
      // const newMap = this.map.setCollisionByExclusion([-1]);
      var debugGraphics = this.add.graphics().setAlpha(0.75);
      this.collidingLayer.renderDebug(debugGraphics, {
        tileColor: null,
      ...
      socket.on('currentPlayers', this.currentPlayers);
      socket.on('newPlayer', this.addOtherPlayers);
      socket.on('playerMoved', this.playerMoved);
      socket.on('disconnect', this.disconnected);
      socket.on('scoreUpdate', this.scoreUpdate);
      socket.on('starLocation', this.starLocation);
    }
  }, {
    key: 'update',
    value: function update(time, delta) {
      ...
      if (this.marker) {
        ...
        if (this.input.activePointer.isDown) {
          console.log('place something here');
    ...
    key: 'starLocation',
    value: function starLocation(_starLocation) {
      var socket = this.socket;


      if (this.star) this.star.destroy();
      this.star = this.physics.add.image(_starLocation.x, _starLocation.y, 'star');
    ...
    key: 'playerMoved',
    value: function playerMoved(playerInfo) {
      this.otherPlayers.getChildren().forEach(function (otherPlayer) {
    ...
    key: 'currentPlayers',
    value: function currentPlayers(players) {
      var socket = this.socket,
          addPlayer = this.addPlayer,
          addOtherPlayers = this.addOtherPlayers;
    ...
    key: 'addPlayer',
    value: function addPlayer(playerInfo) {
      this.player = this.physics.add.sprite(playerInfo.x, playerInfo.y, 'person').setMaxVelocity(300, 400).setOrigin(0.5, 0.5).setDisplaySize(53, 40);
      ...
      this.physics.add.collider(this.player, this.collidingLayer);

      // add map bounds and camera controls
      this.cameras.main.startFollow(this.player);
      this.cameras.main.setBounds(0, 0, this.map.widthInPixels, this.map.heightInPixels);
    }
  }, {
    key: 'addOtherPlayers',
    value: function addOtherPlayers(playerInfo) {
      var otherPlayer = this.add.sprite(playerInfo.x, playerInfo.y, 'otherPlayer').setOrigin(0.5, 0.5).setDisplaySize(53, 40);
      if (playerInfo.team === 'blue') {
...
},{"socket.io-client":"..\\node_modules\\socket.io-client\\lib\\index.js"}],"js\\index.js":[function(require,module,exports) {
'use strict';
...
//# sourceMappingURL=/js.5b9af7c8.map